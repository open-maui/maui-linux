...
public static class LinuxProgramHost
{
    ...
    public static void Run<TApp>(string[] args, Action<MauiAppBuilder>? configure) where TApp : class, IApplication, new()
    {
        ...
        // Single canonical bootstrap path. LinuxApplication.Run owns option resolution,
        // GTK pre-init, theme wiring, CreateWindow handshake, and the event loop.
        LinuxApplication.Run(mauiApp, args, configure: null);
    }

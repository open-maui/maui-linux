...
> Phases 1 and 2 of the Wayland-first roadmap. GPU-native presentation: frames are rasterised by Skia's OpenGL ES backend and handed to the compositor through EGL (zero-copy on Wayland), behind a new render-target seam that keeps the CPU raster path as an automatic fallback; measured on an identical video-playback workload at 1400x1050, per-frame time 3.55 ms -> 1.47 ms average, p99 7-10 ms -> under 4 ms. And the WebView rebuilt on WPE WebKit: composited inside the Skia tree, the first WebView that works in native Wayland/X11 mode, with Blazor Hybrid on top through a new opt-in package.

### Added

- **WPE WebKit WebView** (`Views/WpeWebView.cs`, `Handlers/WpeWebViewHandler.cs`). WPE WebKit 2.54+ renders the page headlessly (GPU-backed through a DRM render node), each frame arrives through WPEPlatform's `buffer-rendered` signal and the view draws it like any other content. No GTK widget, no reparenting, no separate toplevel; identical on native Wayland and X11 and it composes with the GPU render target. Pointer, scroll, keyboard and text input are translated to `WPEEvent`s; the view is sized in CSS pixels with the device scale on its toplevel so pages lay out at the same size as on every other platform (`devicePixelRatio` reports the real scale). Maps `Source`, `UserAgent`, `GoBack`/`GoForward`/`Reload`, `Eval`, and `EvaluateJavaScriptAsync` now returns real results. Navigating (cancellable), Navigated, `CanGoBack`/`CanGoForward` wired through `load-changed`/`decide-policy`.
- **WebView backend selection.** WPE when `libWPEWebKit-2.0` loads, otherwise the existing GTK-hosted WebKitGTK view (unchanged, still requires GTK mode). `OPENMAUI_WEBVIEW=wpe|webkitgtk|auto` overrides. Fedora ships no WPE packages: `dnf copr enable philn/wpewebkit && dnf install wpewebkit` (2.54.0 for Fedora 43/44, x86_64 and aarch64); Debian sid and Ubuntu carry `libwpewebkit-2.0-1`.
- **Context menus in the WebView.** WPE hands the embedder the menu model only; the items (Open Link, Copy Link, Copy/Cut/Paste, Back/Forward/Reload, ...) are shown through the platform's own Skia-drawn context menu (GTK menu in GTK mode) and a chosen item activates WebKit's action.
- **JavaScript dialogs, file chooser and cursor in the WebView.** `alert`/`confirm`/`prompt`/`beforeunload` are shown through the platform's own modal dialogs and answered asynchronously; `prompt` uses a new prompt dialog with a text field (`LinuxDialogService.ShowPromptAsync`, also available to app code; typed text is now routed to modal dialogs). `<input type="file">` opens the platform file picker (single and multiple). The pointer follows the page (hand over links, I-beam over text) by surfacing WPE's cursor requests to the platform cursor.
- **Permissions, web notifications, downloads, spell checking in the WebView.** Geolocation, camera/microphone/screen and notification permission requests are asked through the platform's confirm dialog (anything else is denied); the Web Notifications API is routed to the platform notification service; downloads save into `~/Downloads` under the suggested name (made unique) with a completion/failure notification; spell checking is enabled for the user's locale (red underlines and a "Spelling Suggestions" group in the context menu when enchant and a dictionary are installed). Context-menu submenus are flattened into the platform menu as a header plus indented items. Double and triple clicks now carry a click count, so word and paragraph selection work.
- **Clipboard bridge.** The headless WPE platform has an in-process clipboard; the view syncs it with the system clipboard in both directions (Copy/Cut out via change-count tracking; system text in on focus, secondary click and Ctrl+V) so copy and paste behave like a browser.
- **`WebKitContentApi`** (`Native/WebKitContentApi.cs`, public). The WebKit GLib content API shared by the WPE and WebKitGTK ports (custom URI schemes with per-view routing, user scripts, script-message handlers, `evaluate_javascript`) resolved by symbol from a given library, so bridges are written once.
- **`OpenMaui.Controls.Linux.Blazor`** (new opt-in sibling package). Linux backend for `Microsoft.AspNetCore.Components.WebView.Maui`: `.UseLinuxBlazorWebView()` registers `LinuxBlazorWebViewHandler` over the WPE view with a `WebViewManager` that serves the app through the `app://localhost/` scheme (WebKit rejects `0.0.0.0` as a restricted host), bridges JavaScript messages through a WebKit script-message handler, starts Blazor once the bridge exists (`autostart="false"` host pages), embeds `blazor.webview.js` from the exact framework version it compiles against, serves an empty JS-initializers manifest when the app has none, and raises `BlazorWebViewInitializing`/`Initialized` and `UrlLoading` (external links open in the system browser by default). Root components, DI-injected services, `@onclick` and `@bind` all work. Requires WPE.
- **Samples.** New `BlazorDemo` (MAUI toolbar over a BlazorWebView: counter, two-way binding, injected service, external link). `WebViewDemo` now runs in native mode when WPE is present.
- **Spike records.** `docs/WPE-EMBEDDING.md` with the verified embedding sequence and pitfalls, plus the C probes (`wpe-headless-probe.c`, `wpe-click-probe.c`).
...
### Known gaps (WPE WebView)

- Keyboard events carry no hardware keycode (pages reading `event.code` see none). Frames are copied to a bitmap per frame (the DMA-BUF to EGLImage zero-copy path on the GPU target is the next step).

### Changed

- WebView tests: `tests/Views/WpeWebViewSupportTests.cs` covers keysym translation (printable and control keys, round trip) and backend selection.
...
- **No more dead launcher entries when running from an AppImage.** `InstallDesktopEntry` wrote a `.desktop` file whose `Exec` pointed into the AppImage's ephemeral FUSE mount (`/tmp/.mount_*`) — a broken launcher icon after every run, duplicating the AppImage installer's own entry. The platform now detects `$APPIMAGE` and skips its desktop entry, leaving desktop integration to the AppImage's first-run installer.
- **Closing a WebView app no longer crashes (SIGSEGV) on exit.** Teardown fired a focus-lost invalidation (`WindowContext.Dispose` → `FocusedView = null` → `OnFocusLost` → `RequestRedraw`) into the already-destroyed GTK host window (`gtk_widget_queue_draw` on a freed widget). Root-cause diagnosed from the core dump. Dispose now drops focus/hover/capture silently; every GTK widget pointer is zeroed via its `destroy` signal and all GTK entry points guard on zero; WebView navigation/theme idle sources are tracked and removed on disconnect. Also fixed a latent same-class bug: `SkiaWebView` released its GTK toplevel with `g_object_unref` instead of `gtk_widget_destroy`, leaving the window and WebKit child alive in GTK's toplevel list after managed code considered them gone.
- **Returning to a list page no longer flashes.** Two frame-ordering defects: (a) after an `ItemsSource` refresh, the first frame laid rows out at the default height and a corrective frame re-laid them at measured heights — items are now measured before row positions are computed, so the first frame is final (also fixes the same flash on theme switches); (b) on animated push/pop, `OnAppearing` fired after the animation settled, making appearance-time refreshes visible — it now fires before the page is presented, matching the other MAUI platforms. Note: apps now receive `OnAppearing` before the transition animation instead of after it.

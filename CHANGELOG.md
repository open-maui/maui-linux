...
## [10.0.101.3] - 2026-09-20

> Phase 3 of the roadmap: conformance. A computed compatibility scorecard (`docs/COMPATIBILITY.md`, generated by `tools/Scorecard` from the test run, never hand-edited) measures the platform against the same categories Microsoft publishes for its maui-labs GTK backend, and the gaps it exposed were closed: real handlers for AbsoluteLayout, ContentPresenter, TemplatedView, TableView and ListView; alerts, action sheets and prompts dispatched the way MAUI 10 dispatches them; modal navigation; animations on MAUI's own ticker and animation manager; visual states, triggers and behaviors reaching the platform; FormattedText spans with gestures; gesture recognizers dispatched per MAUI's rules; context flyouts; Essentials services with tests behind an external-process seam; and a golden-screenshot harness at five scale factors. The suite grew from about 900 to 1,415 tests, all passing, run serially by design; the scorecard reports 125 of 125 items covered.

### Added

- **Compatibility scorecard** (`tools/Scorecard`, `docs/COMPATIBILITY.md`). `dotnet run --project tools/Scorecard -- --run` executes the suite and regenerates the document from the TRX: 19 categories, 125 items, each item covered only when at least one mapped test exists and every mapped test passed. `categories.json` maps items to test classes, methods or method-name patterns, so the number is computed, not asserted.
- **Golden screenshot tests** (`tests/Golden/`). Scenes render through a real `SkiaRenderingEngine` over a bitmap target at 1.0x, 1.25x, 1.5x, 1.75x and 2.0x and compare against committed baselines (`tests/Golden/Baselines/`, 60 images) with a per-channel tolerance; mismatches write actual/expected/diff images; `OPENMAUI_UPDATE_GOLDENS=1` re-records. Twelve scenes: labels (sizes, descenders, wrapping, max lines), buttons, entry/editor, toggles, ranges, pickers, grid and border, shapes, formatted text, table view, collection view.
- **Out-of-process WebView conformance** (`tests/WebViewHost`). WebKit binds itself to the process main thread, which xunit never runs tests on, so ten WPE scenarios (HTML and URL sources, navigation events, JavaScript evaluation with real results and errors, cancellable navigation, history with GoBack/CanGoForward, user agent, first frame delivery, cookies over a local HTTP server, reload and stop) run in a small host program launched per scenario by `WebViewHandlerTests`.
- **`AbsoluteLayoutHandler`**: `AbsoluteLayout.LayoutBounds` and `LayoutFlags` (absolute, proportional position and size, AutoSize) applied on `SkiaAbsoluteLayout`; previously AbsoluteLayout resolved to the generic layout handler and stacked its children vertically.
- **`ContentPresenterHandler` and `TemplatedViewHandler`**: `ControlTemplate` on ContentView, ContentPage and TemplatedView renders the template with the content presented inside it; `TemplateBinding` and `GetTemplateChild` work; templates set after realisation re-render.
- **`TableViewHandler` and `SkiaTableView`**: sections (plain and grouped styles), TextCell, SwitchCell, EntryCell, ImageCell and ViewCell rows through a shared `CellViewFactory`, row tap and cell command dispatch, `RowHeight`, header/footer.
- **`ListViewHandler`** over `SkiaCollectionView`: `ItemsSource`, `ItemTemplate` with the standard cells, grouping with header rows, `ItemSelected`/`ItemTapped` in MAUI's order, `SelectedItem` highlight both ways, `ScrollTo`, header/footer, `RowHeight`, observable sources.
- **Dialogs the MAUI 10 way** (`Views/LinuxAlertManager.cs`). `DisplayAlert`, `DisplayActionSheet` and `DisplayPromptAsync` are served through MAUI 10's keyed alert services registered by `UseLinux`, so they work from any page (modal pages included) and return the user's choice; `SkiaActionSheetDialog` and `SkiaModalDialog` (the shared base for alerts, prompts and sheets) draw them.
- **Modal navigation** (`Handlers/WindowHandler.cs`): `Navigation.PushModalAsync`/`PopModalAsync` present the page in a modal layer above the window content; input is routed to the top layer; `ModalPushed`/`ModalPopped` fire.
- **Animations on MAUI's pipeline** (`Hosting/LinuxTicker.cs`, `LinuxAnimationManager.cs`). `ITicker` and `IAnimationManager` are registered in DI so `FadeTo`, `TranslateTo`, `ScaleTo`, `RotateTo`, `LayoutTo`, `RelRotateTo`, `RelScaleTo`, `Animation.Commit`, `AbortAnimation`, `CancelAnimations`, easing and repeat all run through MAUI's own animation manager; the ticker honours the reduce-motion setting (animations complete immediately). Standalone `SkiaView` animations (`SkiaViewAnimationExtensions`) share the same manager.
- **`VisualStateBridge`**: platform focus, pointer-over, pressed and enabled state feed `VisualStateManager` (CommonStates Normal/Focused/PointerOver/Pressed/Disabled) for every view, so XAML visual states, `Trigger`, `DataTrigger`, `MultiTrigger`, `EventTrigger` and style triggers change what is drawn.
- **Behaviors**: `Behavior<T>` attached before or after realisation observes platform text input and taps.
- **FormattedText**: `Label.FormattedText` lays out spans with their own font family, size, attributes, colour, background, decorations, line height and character spacing, and span `GestureRecognizers` receive taps at the span's rectangles.
- **Gesture recognizers dispatched per MAUI**: `GestureManager` was rewritten around MAUI's rules: tap counts and buttons, pan with velocity, pinch, swipe direction and threshold, pointer entered/exited/moved/pressed/released with positions relative to any element, drag-and-drop with `DragStarting`/`DragOver`/`Drop`/`DropCompleted`, and `InputTransparent`/`IsEnabled` respected.
- **Shapes**: `Path` (geometry data and figures), `Line`, `Rectangle` (radii), `Ellipse`, `Polygon`, `Polyline` with fill, stroke, dash arrays and offsets (`Views/ShapeDashing.cs`), caps, joins and `Aspect`; `GraphicsView` drawables.
- **Context flyouts** (`Handlers/ContextFlyoutBridge.cs`). `FlyoutBase.SetContextFlyout(view, menuFlyout)` opens the flyout as the platform context menu on secondary click on that view or any descendant, replacing the control's built-in menu; sub-items are flattened under a heading. `MenuFlyoutItem` commands run once (the menu bar handler used to execute the command and raise `Clicked`, which executed it twice).
- **Fonts**: `LinuxFontRegistrar` and `LinuxFontManager` replace MAUI's defaults in DI (`AddFont` aliases, `MauiFont` items copied next to the app by `build/OpenMaui.Controls.Linux.targets`, named sizes through `LinuxFontNamedSizeService`, fallback per typeface); `FontImageSource` renders registered icon fonts.
- **Semantics**: `SemanticProperties.Description`/`Hint`/`HeadingLevel` and `AutomationId` reach the platform view (`SemanticMapper`) and `SemanticScreenReader.Announce` speaks through the accessibility service when one is present.
- **Essentials**: `FileSystem` (XDG directories, `MauiAsset` reading), `WebAuthenticator` (loopback redirect), and the six sensors without desktop hardware (Accelerometer, Barometer, Compass, Gyroscope, Magnetometer, OrientationSensor) report `IsSupported = false` and throw `FeatureNotSupportedException` on start, as Essentials specifies. Every other service (Launcher, Browser, Email, SMS, PhoneDialer, Map, Share, TextToSpeech, DeviceDisplay, Geolocation, AppInfo settings) spawns external programs through one internal `ExternalProcess` seam, and Battery, Vibration, HapticFeedback and Flashlight read sysfs through a `Sysfs` root, so all 36 Essentials items have real tests that launch nothing and need no hardware. `Screenshot.CaptureAsync` now renders the application's own root view offscreen (PNG or JPEG at the real size and scale) instead of shelling out to grim/gnome-screenshot and capturing the whole desktop; `TextToSpeech.GetLocalesAsync` returns the current UI culture (was empty) and spd-say gets pitch, volume and locale.
- **Typography page** in the ShellDemo sample: named sizes, families, attributes, decorations, spacing and formatted text on one page for visual confirmation.

### Changed

- **Serial test run.** `tests/xunit.runner.json` disables collection parallelism: the platform has process-wide state (`Application.Current`, the GLib main context) and the suite is faster and deterministic serially (about 12 seconds for 1,415 tests).
- Headless test hosts pin `UserAppTheme = Light` so pixel assertions do not follow the desktop theme of the machine running them.
- Handler registry: `MauiHandlerExtensions` and `UseLinux` now register the same handler set (AbsoluteLayout, FlexLayout, shapes, CarouselView, SwipeView, RefreshView, IndicatorView, MenuBar, MenuFlyout, ContentPresenter, TemplatedView, TableView, ListView); `HandlerRegistryTests` fails if they diverge again.
- `FlexLayout` arrange rewritten line by line: `Wrap`, `AlignContent`, `AlignItems`, `JustifyContent`, `AlignSelf`, `Basis`, `Grow`/`Shrink` and reverse directions follow the CSS flexbox algorithm MAUI specifies.
- `AbsoluteLayout` proportional positioning uses MAUI's formula (fraction of the free space, not of the container).

### Removed

- The platform's own `Microsoft.Maui.Platform.Linux.Easing` and `AnimationManager` types, superseded by MAUI's `Easing` and the DI-registered `IAnimationManager`. Code that referenced them switches to `Microsoft.Maui.Easing` and `view.FadeTo(...)` and friends, which now run on MAUI's pipeline. `Handlers/PathHandler.cs` and `Views/SkiaPath.cs` are replaced by `ShapePathHandler`.

### Fixed

- **Views inside layouts never received the render context.** `SkiaLayoutView` (and other containers) keep their own child lists, so the engine's `RenderContext` set on the root never reached descendants; they fell back to `SKTypeface.Default`, which silently ignored `FontFamily` and `FontAttributes`: italic text rendered upright and bold was synthetic emboldening rather than the family's bold face. The context now resolves through the parent chain. Found by the new golden tests on their first run.
- **Font fallback cache conflated styles.** `FontFallbackManager` cached "which typeface renders this codepoint" by family name only, so after a regular label had been drawn, italic and bold labels of the same family were handed the regular face. Keyed by typeface instance now.
- Essentials services (found by the new tests): `Map` formatted latitude/longitude with the current culture (`48,85` under a German locale) and `TryOpenAsync` always returned true; `Launcher` passed the unescaped URI to `xdg-open` (spaces broke it); `PhoneDialer` and `SMS` escaped `+` to `%2B` in `tel:`/`sms:` URIs (invalid per RFC 3966/5724) and `SMS` appended an empty `?body=`; `Email` encoded `@` as `%40` in addresses (RFC 6068 keeps it literal); `FilePicker` faulted its task when `FileTypes` was set (evaluated outside the try on the portable Essentials build); `Battery` reported `PowerSource.Battery` on desktops with no battery (now AC, and a `Mains` supply with `online=1` is honoured); `DeviceDisplay` parsed `GDK_SCALE`/`QT_SCALE_FACTOR` with the current culture; `Flashlight.TurnOnAsync` silently did nothing without a torch (now `FeatureNotSupportedException`, as documented); `AppActions` resolved `$XDG_DATA_HOME` once at type load.
- **`FilePickerFileType.Images`/`Png`/`Jpeg`/`Videos`/`Pdf` threw on first use.** The portable Essentials build initialises those statics from factories that throw `NotImplementedInReferenceAssemblyException`, so `new PickOptions { FileTypes = FilePickerFileType.Images }` raised `TypeInitializationException` and the type stayed dead for the process. `EssentialsPatches` now redirects the factories to Linux-populated types (png/jpg/jpeg/gif/bmp/webp/tiff/svg for images, mp4/mkv/webm/mov/avi/m4v/mpg for videos), which the file and media pickers read as filters.
- **`SkiaCollectionView.SelectedItem` set from code looped forever.** The property-changed handler wrote the property back through `SelectItem`/`ClearSelection`; `BindableObject` defers re-entrant sets of the same property until the outer set returns, so the deferred writes fired after the re-entrancy guard was released and ping-ponged with the handler. Programmatic selection now mirrors into the selection state without writing the property.

## [10.0.101.2] - 2026-09-20
